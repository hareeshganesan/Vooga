...
public class MainPaneModel
{
    ...
    private ArrayList<MyComponent> mySelectedComponent =
        new ArrayList<MyComponent>();
    ...
    {
        if (instance == null) instance = new MainPaneModel();
        return instance;
    }
    ...
    public void dragRectangle (Point2D p)
    {
        ...
        {
            mySelectingRectangle.setFrame(x,
                                          y,
                                          Math.abs(p.getX() - x),
                                          Math.abs(p.getY() - y));
        }
    ...
    public void dragingComponent (Point2D currentpoint)
    {
        ...
        if (mySelectedComponent.contains(draging)) for (MyComponent m : mySelectedComponent)
        {
            if (m != draging)
            {
                Rectangle2D p =
                    new Rectangle2D.Double(m.getBorderX() + dx,
                                           m.getBorderY() + dy,
                                           m.getBorderWidth(),
                                           m.getBorderHeight());
                m.setBorderFrame(p);
