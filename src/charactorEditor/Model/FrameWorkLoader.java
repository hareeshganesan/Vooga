...
public class FrameWorkLoader
{
    ...
    public static FighterBody load (String file) throws FileNotFoundException
    {
        ...
        {}.getType();
        ArrayList<MyComponent> list =
            gson3.fromJson(wholeFile3, collectionType3);
        ...
        FighterBody fighterbody =
            new FighterBody(torso,
                            root.getName(),
...
//		}
        ...
        double myExpAngle =
            Double.valueOf(root.getProperties().get("myExpAngle"));
        long time = Long.valueOf(root.getProperties().get("time"));
        sequence.put(startTime, new Motion(root.getText(),
                                           myExpAngle,
                                           fighterbody,
                                           time));
                                                  ...
                                                  NodeSprite toReturn)
    {
        ...
        if (root.getParent() == null)
        {
            ...
            toReturn =
                new NodeSprite(root.getText(),
                               GraphicsTest.loadImage(root.getImg().toString()),
                               GroupID.getIdFromString(root.getProperties()
                                                           .get("GroupID")),
                               (double) root.getBorderX(),
                               (double) root.getBorderY(),
                               damage);
            if (indicator == "0") return toReturn;
        }
        for (MyComponent m : root.getChildern())
        {
            ...
            ;
            NodeSprite child =
                new NodeSprite(m.getText(),
                               toReturn,

                               GraphicsTest.loadImage(m.getImg().toString()),
                               dx,
                               dy,
                               damage,
                               baseTheta);
        ...
        for (int i = 0; i < root.getChildern().size(); i++)
            buildNodeSpriteTree(root.getChildern().get(i),
                                "1",
