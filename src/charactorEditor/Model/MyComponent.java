...
public class MyComponent
{
    ...
    public MyComponent (Point2D.Double p, int theSort)
    {
        ...
        dragSize =
            new Rectangle2D.Double(border.getMaxX() - 10,
                                   border.getMaxY() - 10,
                                   10,
                                   10);
    }
    ...
    {
        if (properties.keySet().contains(key)) properties.remove(key);
    }
    ...
    public void getOutofTree ()
    {
        if (parent != null)
        {
            if (children.size() != 0) for (MyComponent m : children)
            {
                parent.addChild(m);
                m.setParent(parent);
            }
    ...
    {
        parent = toSet;
        if (toSet != null) parentForFile = toSet.getText();
        else parentForFile = null;
    }
