...
{
    private HashSet<MousePressedState> allStates =
        new HashSet<MousePressedState>();
                                              ...
                                              MouseEvent e)
    {
        if (instance == null) instance =
            new MousePressedState(mainPaneModel, model, e);
        instance.e = e;
        return instance;
    }
    ...
    public MousePressedState getState ()
    {
        for (MousePressedState state : allStates)
            if (state.getState() != null) return state.getState();
        return null;
    }
    ...
    {
        return (!isPuttingComponent() && (mainPaneModel.setDragingSize(model.findComponent(e.getPoint())) == null));
    }
    ...
    {
        return (middleState() && e.getClickCount() < 2 && model.getSetSizeFlag() == true);
    }


    boolean isMiddlePressed ()
    {
        return (middleState() && e.getClickCount() < 2 && model.getSetSizeFlag() == false);
    }
