...
class MainPane extends JPanel implements MouseListener, MouseMotionListener
{
    ...
    public void mousePressed (MouseEvent e)
    {
        ...
        // 第一次放置进来才走这里
        {
            ...
            getNearestPoint();
            outer.componentList.add((outer.focusCMP =
                new MyComponent(nearest, outer.willPut, outer.maxID, outer)));
    ...
    void drawViewComponent (Graphics2D g) throws IOException
    {
        for (int i = 0; i < outer.componentList.size(); i++)
        {
            ...
            {
                BufferedImage img =
                    ImageIO.read(outer.componentList.get(i).img);
        ...
        for (int i = 0; i < outer.componentList.size(); i++)
        {
            ...
            g.drawString(outer.componentList.get(i).text,
                         (int) tem.getX(),
