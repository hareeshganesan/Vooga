...
public class Arrow
{
    ...
    public void draw (Graphics g)
    {
        ...
        {
            double k = getK(ps, pt);
            if (ps.x > pt.x) startAngle =
                360 - (int) (Math.atan(k) * 180 / Math.PI) - 15;
            else startAngle = 180 - (int) (Math.atan(k) * 180 / Math.PI) - 15;
        }
        ...
        if (ps.y < pt.y)
        {// 源在目标上方，目标文字应该在更上面一些，源文字应该更下面一些
            if (ps.x > pt.x) // 目标在源的左方
            g.drawString(getTargetField(),
                         pt.x - fm.stringWidth(getTargetField()),
                         pt.y - ascent - descent);
            else g.drawString(getTargetField(), pt.x, pt.y - ascent - descent);
