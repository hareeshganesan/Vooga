...
public class Model
{
     ...
     * is not already a Sprite at that location or within distance of <radius>
     * 
    ...
    public boolean findLocationConflict (int x, int y, int radius)
    {
        ...
        {
            double xSquared =
                Math.pow(Integer.parseInt(s.getPropertyMap().get("x")) - x, 2);
            double ySquared =
                Math.pow(Integer.parseInt(s.getPropertyMap().get("y")) - y, 2);
