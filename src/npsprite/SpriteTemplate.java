...
public class SpriteTemplate extends Sprite implements Cloneable {
    ...
    public SpriteTemplate clone() {
        ...
        c.addCollisionEvents((HashMap<GroupID, CollisionEvent>) myCollisions
                .clone());
    ...
    public void setNextLocationIncrement(Point2D nextLocation) {
        if (!myCollisionStatus.getStatus()) {
            this.moveBy = new Point2D.Double(moveBy.getX()
                    + nextLocation.getX(), moveBy.getY() + nextLocation.getY());
    ...
    public void update(long elapsedTime) {
        if (this.isActive()) {
            ...
            for (PropertyObject p : myProperties.values()) {
                ...
                if (c != null) {
                    c.performAction(this, null); // BE VERY, VERY CAREFUL -
                                                 // maybe TODO put more safety
                                                 // restrictions
                }
            }
            ...
            moveBy.setLocation(0, 0); // moveBy only work for one time then set
                                      // to
                                      // zero
