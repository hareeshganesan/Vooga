...
{
    private ArrayList<CollisionKind> myReactionList =
                                ...
                                SpriteTemplate spriteTwo)
    {
        ...
        if (getLeft(spriteOne) > getRight(spriteTwo)) return false;
        if (getRight(spriteOne) < getLeft(spriteTwo)) return false;
        if (getUp(spriteOne) > getDown(spriteTwo)) return false;
        if (getDown(spriteOne) < getUp(spriteTwo)) return false;
        return true;
    }
            ...
            for (SpriteTemplate bodyPartTwo : bodyPartsTwo)
            {
                if (isCollided(bodyPartOne, bodyPartTwo))
                {
                    ...
                    for (CollisionKind kind : myReactionList)
                    {
                        ...
                        {
                            kind.doThisReaction(bodyPartOne,
                                                bodyPartTwo,
                                                myPhysicsEngine);
                                          ...
                                          SpriteTemplate spriteTwo)
    {
        ...
        for (SpriteTemplate bodyPartOne : bodyPartsOne)
        {
            for (SpriteTemplate bodyPartTwo : bodyPartsTwo)
            {
                setHorizontalCollision(bodyPartOne,
                                       bodyPartTwo,
                                       statusOne,
                                       statusTwo);
                setHorizontalCollision(bodyPartTwo,
                                       bodyPartOne,
                                       statusTwo,
                                       statusOne);
                setverticalCollision(bodyPartOne,
                                     bodyPartTwo,
                                     statusOne,
                                     statusTwo);
                setverticalCollision(bodyPartTwo,
                                     bodyPartOne,
                                     statusTwo,
                                     statusOne);
            }
    ...
    private void setHorizontalCollision (SpriteTemplate bodyPartOne,
                                         SpriteTemplate bodyPartTwo,
                                         CollisionStatus statusOne,
     ...
     * 
     * @param group the group needs to be set into this collision engine
     */
     ...
     * 
     * @param kindList a list of new collisionKinds
     */
     ...
     * 
     * @param kind the collision kind that needs to be removed
     */
     ...
     * 
     * @param kindList a list of collisionKinds that need to be removed
     */
     ...
     * 
     * @param factor the distance as a standard
     */
