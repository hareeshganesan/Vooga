...
public class Camera extends JPanel
{
    ...
    public void setMinX (int newMin)
    {
        MIN_X = newMin;
    }


    public void setMinY (int newMin)
    {
        MIN_Y = newMin;
    }
    ...
    public void setMaxX (int newMax)
    {
        MAX_X = newMax;
    }


    public void setMaxY (int newMax)
    {
        MAX_Y = newMax;
    }
    ...
    public int getX ()
    {
        return currentBounds.x;
    }


    public int getY ()
    {
        return currentBounds.y;
    }
    ...
    public void calculateNewBounds (ArrayList<FighterBody> sprites)
    {
        ...
        for (int i = 0; i < sprites.size(); i++)
        {
            ...
            double radius =
                Math.sqrt(Math.pow(center.x - sprites.get(i).getX(), 2) +
                          Math.pow(center.y - sprites.get(i).getY(), 2));
        ...
        currentBounds.x =
            (int) (currentBounds.x + ((targetBounds.x - currentBounds.x) / zoomSpeed));
        currentBounds.y =
            (int) (currentBounds.y + ((targetBounds.y - currentBounds.y) / zoomSpeed));
        currentBounds.height =
            (int) (currentBounds.height + ((targetBounds.height - currentBounds.height) / zoomSpeed));
        currentBounds.width =
            (int) (currentBounds.width + ((targetBounds.width - currentBounds.width) / zoomSpeed));
                        ...
                        CameraBackground bg)
    {
        ...
        if (!inSpecialMode)
        {
            ...
            {
                if (sprite.getCollisionStatus().getStatus() &&
                    !sprite.getCollisionStatus().getStandOnSth() &&
